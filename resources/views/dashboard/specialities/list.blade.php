...
@section('title', __('backend.specialitiesOf')." ".$WebmasterSectionTitle)
@section('content')
    <div class="padding">
        <div class="box">
            <div class="box-header dker">
                <h3>{{ __('backend.specialitiesOf') }}  {!! $WebmasterSectionTitle !!}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ __('backend.home') }}</a> /
                    <a>{!! $WebmasterSectionTitle !!}</a> /
                    <a>{{ __('backend.specialitiesOf') }}  {!! $WebmasterSectionTitle !!}</a>
                </small>
            </div>
            @if($Sections->total() > 0)
                @if(@Auth::user()->permissionsGroup->add_status)
                    <div class="row p-a">
                        <div class="col-sm-12">
                            <a class="btn btn-fw primary" href="{{route("specialitiesCreate",$WebmasterSection->id)}}">
                                <i class="material-icons">&#xe02e;</i>
                                &nbsp; {{ __('backend.specialityNew') }}</a>
                        </div>
                    </div>
                @endif
            @endif
                        ...
                        <div class=" p-a text-center ">
                            ...
                            @if(@Auth::user()->permissionsGroup->add_status)
                                <a class="btn btn-fw primary"
                                   href="{{route("specialitiesCreate",$WebmasterSection->id)}}">
                                    <i class="material-icons">&#xe02e;</i>
            ...
            @if($Sections->total() > 0)
                {{Form::open(['route'=>['specialitiesUpdateAll',$WebmasterSection->id],'method'=>'post'])}}
                                ...
                                <td class="text-center">
                                    ...
                                    @if(@Auth::user()->permissionsGroup->edit_status)
                                        <a class="btn btn-sm success"
                                           href="{{ route("specialitiesEdit",["webmasterId"=>$WebmasterSection->id,"id"=>$Section->id]) }}"
                            ...
                            @foreach($Section->fatherSections as $subSection)
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @if(@Auth::user()->permissionsGroup->edit_status)
                                            <a class="btn btn-sm success"
                                               href="{{ route("specialitiesEdit",["webmasterId"=>$WebmasterSection->id,"id"=>$subSection->id]) }}"
                                               data-toggle="tooltip" data-original-title="{{ __('backend.edit') }}">
                                                <i class="material-icons">&#xe3c9;</i>
                                            </a>
                                        @endif
                                ...
                                @foreach($subSection->fatherSections as $subSection2)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            @if(@Auth::user()->permissionsGroup->edit_status)
                                                <a class="btn btn-sm success"
                                                   href="{{ route("specialitiesEdit",["webmasterId"=>$WebmasterSection->id,"id"=>$subSection2->id]) }}"
                                                   data-toggle="tooltip" data-original-title="{{ __('backend.edit') }}">
                                                    <i class="material-icons">&#xe3c9;</i>
                                                </a>
                                            @endif
...
@push("after-scripts")
    <script type="text/javascript">
        ...
        function DeleteCategory(id) {
            $("#category_delete_btn").attr("href", '{{ route("specialitiesDestroy",["webmasterId"=>$WebmasterSection->id]) }}/' + id);
